    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[70] p-4">
      <div className="bg-slate-800 rounded-xl w-full max-w-lg border border-slate-700 shadow-2xl">
        ...
        <div className="p-4 space-y-3">
          <p className="text-[10px] text-amber-300 leading-relaxed bg-amber-500/5 border border-amber-500/20 rounded p-2">
          ...
          {/* 预设工作流 */}
          <div className="space-y-1">
            <label className="text-xs text-slate-400">预设工作流</label>
            <select
              value=""
              onChange={(e) => {
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              rows={8}
              className="w-full text-[10px] font-mono rounded border border-slate-600 bg-slate-700 px-2 py-1.5 text-white placeholder:text-slate-500 resize-y"
            />
          </div>

          {/* 校验结果 */}
          {validation && (
            <div className={`text-[9px] rounded p-1.5 space-y-0.5 ${validation.ok ? 'bg-emerald-500/10 text-emerald-300' : 'bg-red-500/10 text-red-300'}`}>
              {validation.errors.map((e, i) => <p key={`e${i}`}>✕ {e}</p>)}
              {validation.warnings.map((w, i) => <p key={`w${i}`} className="text-amber-400">⚠ {w}</p>)}
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-slate-700">
          ...
          <button
            onClick={handleSave}
            className="px-4 py-1.5 text-xs bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 rounded transition-colors"
          >
            保存工作流
          </button>
        </div>
