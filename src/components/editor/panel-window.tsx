...
export function PanelWindow({ graph }: PanelWindowProps) {
  ...
  return (
    <div className="h-screen w-full bg-card text-foreground overflow-hidden">
      <div className="h-full flex flex-col">
        <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-card/95 backdrop-blur-sm">
          <span className="text-sm font-medium text-foreground">管理面板</span>
          <button
            onClick={() => {
              ;(window as unknown as { __electronAPI?: { closePanelWindow: () => void } })
                .__electronAPI?.closePanelWindow()
            }}
            className="text-xs px-2 py-1 rounded bg-muted hover:bg-secondary text-muted-foreground transition-colors"
          >
            关闭
          </button>
        </div>
        <div className="flex-1 overflow-hidden p-4">
          <div className="text-center text-muted-foreground mt-20">
            ...
            <p className="text-xs mt-1 text-muted-foreground">按 P 键或 Cmd/Ctrl+W 关闭</p>
          </div>
        </div>
      </div>
    </div>
