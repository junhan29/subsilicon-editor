...
export function AiAssistButton({ mode, context, onResult, className = '', size = 'sm', label }: AiAssistButtonProps) {
  ...
  const handleClick = async () => {
    ...
    if (!available) {
      ...
      showToast('error', `${getAssistantName()}未配置，请先在${getAssistantName()}设置中配置 API`)
      return
    }
    ...
    try {
      if (mode === 'image' || mode === 'video') {
        showToast('info', '图片/视频生成需要通过创作助理媒体服务调用')
        return
      }
      ...
      if (e instanceof Error && 'needsConfig' in e && (e as { needsConfig: boolean }).needsConfig) {
        setShowSettings(true)
        showToast('error', `${getAssistantName()}未配置，请先设置 API Key 或启动本地 Ollama`)
      } else {
