...
export function checkDomainBinding(): boolean {
  ...
  if (!isAllowed) {
    ...
    console.warn('如需授权使用，请联系：contact@subsilicon.cn')

    // 上报盗用信息（如果有的话）
    reportPiracy(hostname, window.location.href)
 ...
 * 上报盗用信息
 */
async function reportPiracy(hostname: string, url: string) {
  try {
    // 静默上报，不阻塞
    const reportUrl = 'https://subsilicon.cn/api/piracy-report'
