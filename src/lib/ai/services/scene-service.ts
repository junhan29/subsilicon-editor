...
export async function generateSceneSimple(
  ...
  const result = await callAiForTask('text',
    {
      systemPrompt: `${systemPrompt}${buildGenerationPolicyPrompt(`${location}：${atmosphere}`, context)}`,
      userPrompt,
      temperature: 0.7,
      maxTokens: 200,
    }
...
export async function generateSceneDescription(
  ...
  const rawResult = await callAiForTask('text',
    {
      systemPrompt: `${SCENE_DESCRIPTION_SYSTEM_PROMPT}${buildGenerationPolicyPrompt(`${location}：${mood}`, context)}`,
      userPrompt,
      temperature: 0.7,
      maxTokens: 600,
    }
  ...
  try {
    ...
    return {
      ...
      backgroundImage: deaiStyle(typeof parsed.backgroundImage === 'string' ? parsed.backgroundImage : ''),
      mood: deaiStyle(typeof parsed.mood === 'string' ? parsed.mood : mood),
      lighting: deaiStyle(typeof parsed.lighting === 'string' ? parsed.lighting : ''),
      result: rawResult,
    }
  } catch {
    return {
      description: deaiStyle(rawResult),
